...
// Altera a forma de pagamento da assinatura ativa do usuário logado
function alterarFormaPagamento(novaForma, usuario = App.usuario) {
  const id = getIdentificadorUsuario(usuario);
  if (!id) return false;
  ...
  todas[id].formaPagamento = novaForma;
  salvarAssinaturasSalvas(todas);
  return true;
}
window.alterarFormaPagamento = alterarFormaPagamento;
